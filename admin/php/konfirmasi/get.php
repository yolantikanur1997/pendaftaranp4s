...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
   ...
   $stmt->bind_result($id,$no_rekening,$atas_nama,$bank,$tanggal_bayar,$tanggal_konfirmasi,$status,$catatan,$no_pengajuan,$harga,$id_pengajuan_peserta,$nama_instansi);

    if ($stmt->execute()) {

        while($stmt->fetch())
        {
            $output = array("id_konfirmasi"=>$id_konfirmasi,"no_rekening"=>$no_rekening,"atas_nama"=>$atas_nama,"bank"=>$bank,"tanggal_bayar"=>$tanggal_bayar,"tanggal_konfirmasi"=>$tanggal_konfirmasi,"status"=>$status,"catatan"=>$catatan,"no_pengajuan"=>$no_pengajuan,"harga"=>$harga,"id_pengajuan_peserta"=>$id_pengajuan_peserta,"nama_instansi"=>$nama_instansi);
        }
