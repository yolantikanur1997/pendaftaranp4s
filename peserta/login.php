...
$data = mysqli_query($koneksi,"select * from registrasi_akun_instansi where username='$username'");
...
$data2 = mysqli_fetch_array($data);
  ...
  $_SESSION['nama'] = $data2['nama_instansi'];
  $_SESSION['username'] = $data2['username'];
  // $_SESSION['nama_instansi'] = $data2['nama_instansi'];
...
if(password_verify($password, $data2['password']) ){
  ...
  echo "<script>window.location='../dashboardpeserta/index.php'</script>"; 
