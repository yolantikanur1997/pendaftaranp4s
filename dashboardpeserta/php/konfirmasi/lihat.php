...
$stmt->bind_result($id,$no_rekening,$atas_nama,$bank,$tanggal_bayar,$tanggal_konfirmasi,$status,$catatan,$no_pengajuan);

if ($stmt->execute())
{
    while($stmt->fetch())
    {
        $output[] = array("id"=>$id,"no_rekening"=>$no_rekening,"atas_nama"=>$atas_nama,"bank"=>$bank,"tanggal_bayar"=>$tanggal_bayar,"tanggal_konfirmasi"=>$tanggal_konfirmasi,"status"=>$status,"catatan"=>$catatan,"no_pengajuan"=>$no_pengajuan);
    }
